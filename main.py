...
def get_daily_data(word: str,
    ...
    with open("allCode/data_code/keywords.txt", "r") as f:
        for line in f:
            keyword = line.strip()
            keyword_file =keyword.replace(' ', '_')
            # Initialize build_payload with the word we need data for
            build_payload = partial(pytrends.build_payload,
                                    kw_list=[keyword], cat=0, geo=geo, gprop='')
            ...
            complete[f'{keyword}_monthly'].ffill(inplace=True)  # fill NaN values
            complete['scale'] = complete[f'{keyword}_monthly'] / 100
            complete[keyword] = complete[f'{keyword}_unscaled'] * complete.scale

            complete.to_csv(
                rf'data/daily_full/{keyword_file}.csv')

            # return complete


urllib3.disable_warnings()
print(get_daily_data('ball', 2020, 1, 2020, 1))
